...
const pageCopy = {
  ...
  my: {
    ...
    subtitle: "Repair and checking tasks currently assigned to you.",
  },
  done: {
...
export default function RepairDeviceWorkflowPage({ mode, onOpenRecord, userEmail }) {
  ...
  useEffect(() => {
    ...
    async function loadRepairRecords() {
      ...
      // New Repair Device shows unassigned tasks that need someone to get the repair/checking work.
      if (mode === "new") {
        query = query.is("assigned_to", null).neq("workflow_status", "Done Repair Device");
      }

      // My Repair Device shows records assigned to the signed-in user.
      if (mode === "my") {
  ...
  const handleGetRepair = async (record) => {
    ...
    const repairByName = getUserDisplayName(user, assignedEmail);

    // Update assignment fields so this row moves from New Repair Device to My Repair Device.
    ...
    // Mirror Repair By into Ongoing Testing so the generated testing table shows the assigned person.
    ...
    await logAuditEvent({
      ...
      summary: `${repairByName || "User"} got repair task ${getRecordLabel(record)}.`,
    });

    // Remove the row from the New Repair Device queue after it is assigned.
        ...
        >
          <TableHead>
            <TableRow>
              ...
              {mode === "new" || mode === "done" ? <TableCell align="center" width={mode === "done" ? 90 : 130}>Action</TableCell> : null}
            </TableRow>
          </TableHead>
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={mode === "new" || mode === "done" ? 11 : 10} align="center" sx={{ py: 4 }}>
            ...
            {records.map((record) => (
              <TableRow
                ...
                onDoubleClick={() => {
                  if (mode === "my" || mode === "done") onOpenRecord(record.id);
                }}
                sx={{ cursor: mode === "my" || mode === "done" ? "pointer" : "default" }}
              >
